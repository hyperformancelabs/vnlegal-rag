...
Run from repo root:
  python experiments/build_shared_embedding.py
"""
...
def detect_data_dir() -> Path:
    """Locate data_ready (same candidates as training notebooks)."""
    candidates = [
        Path("data/data_ready"),
        Path("../data/data_ready"),
        PIPELINE_DIR.parent / "data" / "data_ready",
    ...
    raise FileNotFoundError(
        "Cannot find data_ready with qa_train.csv and corpus_train.csv. "
        "Run: python prepare_data.py --out-dir data/data_ready "
        "(Colab: place data under /content/data/data_ready/)"
    )
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Shared embedding for  QA + corpus train split.")
