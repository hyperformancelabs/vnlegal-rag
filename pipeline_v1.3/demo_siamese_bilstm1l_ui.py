"""
Browser demo for pipeline v1.3 Siamese BiLSTM (1 layer).
...
Configuration:
  ...
  ``pipeline_v1.3/`` directory will be loaded automatically at startup. See
  ``pipeline_v1.3/.env.example`` for the available keys.
"""
...
sys.path.insert(0, str(REPO_ROOT / "pipeline_v1.3"))
DEFAULT_ARTIFACT_DIR = REPO_ROOT / "pipeline_v1.3" / "siamese_bilstm1L_artifacts"
DEFAULT_TEXTCNN_ARTIFACT_DIR = REPO_ROOT / "pipeline_v1.3" / "textcnn_artifacts"
_corpus_full = REPO_ROOT / "data" / "data_ready_v1_3" / "corpus_ready_full.csv"
_corpus_train = REPO_ROOT / "data" / "data_ready_v1_3" / "corpus_train.csv"
...
def _bootstrap_env() -> None:
    ...
    Preference order (later files override earlier ones only if values are missing):
      1. ``<repo_root>/.env``
      2. ``<script_dir>/.env`` (i.e. ``pipeline_v1.3/.env``)
...
<head>
  ...
  <title>VNLegal v1.3 - Siamese BiLSTM</title>
  <style>
    :root {
      --primary: #2563eb;
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="HTML demo for Siamese BiLSTM 1-layer (pipeline v1.3).")
