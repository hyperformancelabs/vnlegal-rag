"""
Bootstrap import for the pipeline v1.3 tokenizer.

Loads ``tokenizer.py`` from the first available location:

  1. ``pipeline_v1.3/tokenizer.py`` next to this file (Colab / Kaggle notebook cwd)
...
def locate_tokenizer_path(start: Path | None = None) -> Path:
    """Return the path to ``tokenizer.py`` used by pipeline v1.3."""
    ...
    for root in _search_roots(start):
        candidates.append(root / "pipeline_v1.3" / "tokenizer.py")
    ...
    raise FileNotFoundError(
        "Cannot locate tokenizer.py. Expected pipeline_v1.3/tokenizer.py "
